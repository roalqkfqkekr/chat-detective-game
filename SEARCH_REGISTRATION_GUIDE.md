...
4. 사이트 등록에 다음 주소를 입력합니다.
   ...
   `https://playatelier.github.io`

5. 실제 서비스 주소는 다음 하위 경로입니다.

   `https://playatelier.github.io/chat-detective-game/`

6. GitHub Project Pages는 하위 경로 사이트이므로 네이버 인증이 까다로울 수 있습니다.
7. 우선 `HTML 태그` 인증 방식을 사용합니다.
8. 현재 `index.html`의 `<head>`에는 다음 인증 태그가 추가되어 있습니다.

   ```html
   <meta name="naver-site-verification" content="94d2527ed160cfa3a66370cd31ba8fdcbd39af4f">
   ```

9. 배포된 HTML 소스에서 위 meta 태그가 보이는지 확인합니다.
10. 네이버 서치어드바이저에서 소유확인을 완료합니다.
11. 사이트맵 제출 메뉴에 다음 주소를 제출합니다.

    `https://playatelier.github.io/chat-detective-game/sitemap.xml`

12. 웹페이지 수집 요청에 다음 주소를 제출합니다.
